...
const FindRecipes = () => {
    ...
    const applyFilters = useCallback(() => {
      ...
      if (selectedItems.length > 0) {
        // Ensure selectedItems are valid and have a name property
        const ingredientNames = selectedItems
          .filter(ing => typeof ing === 'string') // Ensure it's a string
          .map(ing => ing.toLowerCase());
    
        filtered = filtered.filter(recipe => {
          // Check if recipe.ingredients exists and is an array
          if (!recipe.ingredients || !Array.isArray(recipe.ingredients)) return false;
    
          // Extract ingredient names from recipe ingredients, ensuring they exist
          const recipeIngredients = recipe.ingredients
            .filter(ing => ing && typeof ing.name === 'string') // Ensure name is a string

...
const NavSearchBox = ({ setSearchOpen }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-start justify-start p-6 bg-black bg-opacity-50 z-50">
      <form
        onSubmit={(e) => e.preventDefault()}
        className="relative flex flex-col items-center bg-white border border-gray-200 rounded-xl shadow-2xl p-6 w-full max-w-md transition-all duration-300"
      >
       {/* Search Bar */}
         <div className="flex w-full items-center bg-gray-50 border border-gray-200 rounded-full p-2 shadow-sm">
          <input
            type="text"
            placeholder="Search for Friends..."
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            className="w-full px-4 py-2 outline-none bg-transparent text-gray-800 text-lg rounded-full placeholder-gray-400"
          />
          <button
            type="submit"
